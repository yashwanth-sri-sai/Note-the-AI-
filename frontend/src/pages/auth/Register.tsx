...
import { BrainCircuit, Loader2, Mail, Lock, User, AlertCircle, ArrowLeft } from "lucide-react";
...
export const Register: React.FC = () => {
  const { register: signup, googleLogin } = useAuthStore();
  ...
  const onSubmit = async (data: RegisterSchemaType) => {
    ...
    try {
      ...
      setErrorMsg(
        err.response?.data?.detail || "Email already registered. Try logging in."
      );
    } finally {
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-background px-4 py-12 transition-colors duration-300 relative">
      ...
      <div className="w-full max-w-md space-y-6 glass-panel p-8 rounded-2xl shadow-xl border border-border/80">
        {/* Header */}
